...
import { DarazShipmentTracking } from "@/interfaces/daraz.interface";
...
export default function Logistics({
  ...
  ordersTraceInfo: DarazShipmentTracking[];
}) {
  ...
  useEffect(() => {
    ...
    if (courierFilter !== "all") {
      filtered = filtered.filter(
        (shipment) => shipment.courierService === courierFilter
      );
    }
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(
        (shipment) => shipment.courierStatus === statusFilter
      );
    }
      ...
      <div className="ml-64 p-8">
        <div className="mb-8">
          ...
          <p className="text-slate-600 mt-2">
            Track shipments and manage courier services across all platforms.
          </p>
        </div>
            ...
            <Card key={service.name}>
              <CardContent className="pt-6">
                ...
                <p className="text-sm text-muted-foreground">
                  {service.activeShipments} active shipment
                  {service.activeShipments !== 1 ? "s" : ""}
                </p>
              </CardContent>
            </Card>
          ))}
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredShipments.map((shipment) => (
                    <tr
                      key={`${shipment.orderId}-${shipment.trackingNumber}`}
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className="text-lg">
                            {getPlatformIcon(shipment.platform)}
                          </span>
                          ...
                          <span className="text-lg">
                            {
                              courierServices.find(
                                (s) => s.name === shipment.courierService
                              )?.logo
                            }
                          </span>
                      ...
                      <td className="p-4">
                        <Badge
                          variant={
                            getStatusColor(shipment.courierStatus) as any
                          }
                      ...
                      <td className="p-4">
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              handleReassignCourier(shipment.orderId)
                            }
